    ...
    <x-dialog wire:model="paletteShown">
        <x-slot name="title">
            <div class="flex items-center pb-3">
                <input id="command-palette-input" wire:model="search" wire:keyup.enter="runCommand" type="text" class="bg-slate-800 appearance-none text-gray-200 text-sm font-bold flex-grow pb-1 px-3 leading-tight focus:outline-none focus:shadow-outline" placeholder="What would you like to do?" autofocus>
        ...
        <x-slot name="content" class="border-y border-slate-700 divide-y divide-gray-800">
            @forelse ($this->commands as $key => $command)
                <div class="">
                    <button :class="focusedCommand === {{ $key }} && 'bg-gray-700'" class="px-2 py-3 w-full text-left ring-none outline-none appearance-none">{{ $command['name'] }}</button>
                </div>
            @empty
        ...
        <x-slot name="footer">
            <div class="flex items-center space-x-2 w-full text-gray-300 text-xs">
